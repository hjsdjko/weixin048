	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjijiukecheng`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjijiukecheng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussjijiukecheng`, this.ruleForm);
						}else{
							await this.$api.add(`discussjijiukecheng`, this.ruleForm);
						}
						uni.setStorageSync('discussjijiukechengCleanType',true);
